<div class="box-gradient box-gradient-{{ $lightGCNRecommendedProducts->isNotEmpty() ? 'filled' : 'empty' }}
    ...
    <div class="p-6 bg-gradient rounded-2xl bg-white relative z-10"
        ...
        @if ($lightGCNRecommendedProducts->isEmpty())
        ...
        @elseif ($lightGCNRecommendedProducts->count() > 1 && $lightGCNRecommendedProducts->count() <= 10)
            <ul class="grid  grid-cols-5 py-5 gap-5 cursor-pointer ">
                @include('frontend.partials.filtered-product-list', ['products' => $lightGCNRecommendedProducts])
            </ul>
        @else
