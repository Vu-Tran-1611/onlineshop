<div class="box-gradient box-gradient-{{ $recommendedProducts->isNotEmpty() ? 'filled' : 'empty' }}
    my-8 rounded-2xl shadow-2xl">

    {{-- Content --}}
    <div class="p-6 bg-gradient rounded-2xl bg-white relative z-10"
        style="--box-height: {{ $recommendedProducts->isNotEmpty() ? '99%' : '95%' }};">
        ...
        @elseif ($recommendedProducts->count() > 1 && $recommendedProducts->count() <= 10)
            <ul class="grid  grid-cols-5 py-5 gap-5 cursor-pointer ">
                @include('frontend.partials.filtered-product-list', ['products' => $recommendedProducts])
            </ul>
